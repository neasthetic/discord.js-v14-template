...
const registerSlashCommands = async (client, commands, context = "") => {
  try {
    ...
    if (registerType === "GLOBAL") {
      Logger.info(
        `(SLASH) Registrando ${commands.length} comandos globalmente...`,
      );
    ...
    if (!allowedGuilds.length) {
      Logger.warn(
        "(SLASH) Modo GUILD ativado mas nenhum servidor configurado em COMMANDS.SERVER_GUILDS.",
      );
      return;
    }
    ...
    console.log("\n");
    Logger.system(
      `(SLASH) Iniciando sincronização em ${allowedGuilds.length} servidores restritos.`,
    );
    ...
    for (const guildId of allowedGuilds) {
      ...
      if (!guild) {
        Logger.warn(
          `(SLASH) Servidor ${guildId} não encontrado ou bot sem acesso.`,
        );
        continue;
      }
      ...
      await guild.commands
        ...
        .then(() => {
          Logger.success(
            `(SLASH) Sincronizado em: ${guild.name} (${guild.id})`,
          );
        })
        .catch((err) => {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (cmdImport instanceof ApplicationCommand) {
        ...
        typeof cmdImport === "function" &&
        cmdImport.prototype instanceof ApplicationCommand
      ) {
        cmd = new cmdImport();
      }
      ...
      if (
        ...
        Logger.warn(
          `[${moduleName}] Comando inválido em ${file}: deve ter 'name' como string.`,
        );
        continue;
      }
      ...
      Logger.error(
        `[${moduleName}] Erro ao carregar comando ${file}: ${err.message}`,
      );
    }
  }
  return count;
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (cmdImport instanceof PrefixCommand) {
        ...
        typeof cmdImport === "function" &&
        cmdImport.prototype instanceof PrefixCommand
      ) {
        cmd = new cmdImport();
      }

      const meta = cmd?.command || cmd;
      const commandName = String(meta?.name || "")
        .trim()
        .toLowerCase();
      const run = meta?.run;
      ...
      if (!commandName || typeof run !== "function") {
        Logger.warn(
          `[${moduleName}] Prefix command inválido em ${file}: precisa de name e run.`,
        );
        continue;
      }
      ...
      Logger.error(
        `[${moduleName}] Erro ao carregar prefix command ${file}: ${err.message}`,
      );
    }
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (typeof mod === "function") {
        ...
        modImport instanceof Event ||
        (typeof modImport === "function" &&
          modImport.prototype instanceof Event)
      ) {
        ...
        Logger.warn(
          `[${moduleName}] Evento inválido em ${file}: deve ser função, classe ou objeto com execute.`,
        );
        continue;
      }
      ...
      Logger.error(
        `[${moduleName}] Erro ao carregar evento ${file}: ${err.message}`,
      );
    }
  }
  return count;
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (typeof script !== "function") {
        Logger.warn(
          `[${moduleName}] Script inválido em ${file}: deve exportar uma função (module.exports = function (client) { ... }).`
            .yellow,
        );
        continue;
      }
...
module.exports = (
  client,
  modulesPath = DEFAULT_MODULES_PATH,
  reload = false,
) => {
  ...
  const prefixCommandsEnabled = Boolean(
    settings?.COMMANDS?.PREFIX_COMMANDS_ENABLED,
  );
  ...
  if (client.__modulesLoaded && !reload) {
    Logger.info(
      "[MÓDULOS] Módulos já carregados. Use reload=true para recarregar.",
    );
    return;
  }
  ...
  for (const moduleName of entries) {
    ...
    const modulePrefixCmds = prefixCommandsEnabled
      ? LoadPrefixCommands(client, path.join(base, "prefix"), moduleName) +
        LoadPrefixCommands(
          client,
          path.join(base, "prefixCommands"),
          moduleName,
        )
      : 0;
    const moduleEvents = LoadEvents(
      client,
      path.join(base, "events"),
      moduleName,
    );
    const moduleScripts = loadScripts(
      client,
  ...
  perModuleSummary.forEach((m, i) => {
    console.log(
      ` ${String(i + 1).white}. ${m.moduleName.white}: ` +
